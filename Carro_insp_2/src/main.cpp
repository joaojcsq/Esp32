...
void setup() {
  ...
  */
  ...
  ledcWrite(0, 200); //160 --> menor pwm para girar o motor // Valor do PWM no canal 0
  ledcWrite(1, 200); // Valor do PWM no canal 1
}
...
if(radio.available()){ // Se receber algum dado pelo NRF240L1
  ...
  if (entrada[0] == 0){ // Se o valor da posição 0 em "entrada" for igual a 0.
    frente(); // Move o carro para frente.
  }
  if (entrada[0] == 2){
    tras();
  }
